...
const thermal_mod = @import("thermal.zig");
...
pub const AStarRouter = struct {
    ...
    // ── P1-D: LDE keepout integration ────────────────────────────────────────
    /// Precomputed LDE keepout rectangles (generated by LDEConstraintDB.generateKeepouts).
    /// When a candidate node's world position falls inside a keepout, an extra
    /// penalty is added to the step cost to discourage routing through LDE-restricted
    /// zones.  Null disables LDE cost (non-analog or no LDE data).
    lde_keepouts: ?[]const lde_mod.Rect,
    /// Penalty (µm) added per LDE keepout region entered.
    lde_penalty: f32,

    // ── P1-E: Thermal map integration ────────────────────────────────────────
    /// Thermal map for the die.  When set, candidate nodes are queried for local
    /// temperature and a detour cost is added proportional to (T - T_ambient).
    /// Non-analog nets should leave this null (set via setThermalMap).
    thermal_map: ?*const thermal_mod.ThermalMap,
    /// Scale factor for thermal penalty: cost += thermal_weight * (T - T_ambient).
    /// Typical value 0.1 µm/°C.  Set to 0 for non-analog nets.
    thermal_weight: f32,
    ...
    /// Check if a node is within 1 track of the target on the same layer.
    fn isNearTarget(_: *const AStarRouter, node: GridNode, target: GridNode) bool {
        if (node.layer != target.layer) return false;
        const da = if (node.track_a >= target.track_a) node.track_a - target.track_a else target.track_a - node.track_a;
        const db = if (node.track_b >= target.track_b) node.track_b - target.track_b else target.track_b - node.track_b;
        return da <= 1 and db <= 1 and (da + db) > 0;
    }
    ...
    /// Fixed-capacity neighbor buffer (2 preferred + 2 cross + 2 via + 8 via offsets).
    ...
    fn getNeighbors(self: *const AStarRouter, node: GridNode, grid: *const MultiLayerGrid, net: NetIdx) NeighborBuf {
        ...
        // For via-down: also try ±1 track offsets to handle grid quantization
        // between layers with different track structures.
        ...
        if (layer > 0) {
            ...
            const base_via_cost = self.via_cost * @max(1.0, upper_p / lower_p);

            // Try the center landing and ±1 offsets on both track axes.
            const offsets = [_]i32{ 0, -1, 1 };
            for (offsets) |da| {
                for (offsets) |db| {
                    if (buf.count >= buf.items.len) break;
                    const new_a_i: i64 = @as(i64, @intCast(below_node.track_a)) + da;
                    const new_b_i: i64 = @as(i64, @intCast(below_node.track_b)) + db;
                    if (new_a_i < 0 or new_b_i < 0) continue;
                    const new_a: u32 = @intCast(new_a_i);
                    const new_b: u32 = @intCast(new_b_i);
                    if (new_a >= grid.layers[below].num_tracks) continue;
                    if (new_b >= grid.cross_layers[below].num_tracks) continue;
                    const candidate = GridNode{ .layer = below, .track_a = new_a, .track_b = new_b };
                    if (grid.isCellRoutable(candidate, net)) {
                        const offset_penalty: f32 = if (da == 0 and db == 0) 0.0 else lower_p * 0.5;
                        var step_cost = base_via_cost + offset_penalty;
                        const skip = self.drcFilter(grid, candidate, net, &step_cost);
                        if (!skip) {
                            buf.items[buf.count] = .{ .node = candidate, .step_cost = step_cost };
                            buf.count += 1;
                        }
                    }
                }
    ...
    /// Also applies LDE keepout penalty (P1-D) and thermal detour cost (P1-E).
    fn drcFilter(self: *const AStarRouter, grid: *const MultiLayerGrid, nbr: GridNode, net: NetIdx, step_cost: *f32) bool {
        ...
        const wx = world_pos[0];
        const wy = world_pos[1];

        // ── Inline DRC check ─────────────────────────────────────────────────
        if (self.drc_checker) |drc| {
            const result = drc.checkSpacing(nbr.layer, wx, wy, net);
            if (result.hard_violation) return true;
            step_cost.* += self.drc_weight * result.soft_penalty;
        }

        // ── P1-D: LDE keepout penalty ─────────────────────────────────────────
        // Penalise routing through LDE-restricted zones.  The cost discourages
        // paths that alter SA/SB spacing for adjacent matched devices.
        if (self.lde_keepouts) |keepouts| {
            for (keepouts) |ko| {
                if (wx >= ko.x1 and wx <= ko.x2 and wy >= ko.y1 and wy <= ko.y2) {
                    step_cost.* += self.lde_penalty;
                    break; // at most one penalty per step
                }
            }
        }

        // ── P1-E: Thermal map detour cost ─────────────────────────────────────
        // Add cost proportional to excess temperature above ambient.
        // thermal_weight == 0 for non-analog nets (no cost).
        if (self.thermal_map) |tmap| {
            if (self.thermal_weight > 0.0) {
                const temp = tmap.query(wx, wy);
                const excess = temp - tmap.ambient;
                if (excess > 0.0) {
                    step_cost.* += self.thermal_weight * excess;
                }
            }
        }
